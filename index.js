...
const createParticle = (x, y) => {
    ...
    particle.setAttribute("src", "cookie.png");
...
autoClick.addEventListener("click", () => {
    ...
    if (scoreValue >= price) {
        ...
        document.querySelector(".auto-clicks .cursors").innerHTML += '<img src="cursor.png" alt="cursor" id="cursor" class="cursor auto">'
...
/*zweite x2 click button funktion*/
...
const getSavedData = () => {
    ...
    if(storage.powerups.includes("upgrade-clicks")) {
        const multiplier = storage.powerups.filter(powerup => powerup == "upgrade-click").length;
    ...
    if(storage.powerups.includes("auto-clicks")) {
        const quantAutoClicks = storage.powerups.filter(powerup => powerup == "auto-click").length;

        document.querySelector(".auto-click").classList.remove("disable")
